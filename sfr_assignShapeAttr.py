...
class assignAttr(object):
	...
	def __init__(self):
		...
		self.colorSlider = 'sfr_colorSlider'
		# query UI attr
	...
	def sfr_queryAll(self):
		...
		self.useTheCol = cmds.checkBox(self.colorSlider, q = 1, v = 1)
		...
		return self.float3X, self.float3Y, self.float3Z, self.int, self.useTheCol
		return self.attrName, self.checkVal, self.minV, self.maxV, self.attType, self.renderer 

## turn on or off the various checkBoxes
	def sfr_turnOffCheckVam(self,args):
		self.checkVal = cmds.checkBox(self.checkBox, q = 1, v = 1)
		self.colorVal = cmds.checkBox(self.colorSlider, q = 1, v = 1)
		if not self.checkVal == 0:
			cmds.checkBox(self.checkBox, e = 1, v = 0)
		
		cmds.checkBox(self.colorSlider, e = 1, v = 0)
	...
	def sfr_turnOffRandInt(self,args):
		self.checkColSlider =  cmds.checkBox(self.colorSlider, q = 1, v = 1)
		if not self.checkColSlider == 0:
			cmds.checkBox(self.useFloat3Val, e = 1, v = 0)
		cmds.checkBox(self.checkBox, e = 1, v = 0)
		cmds.checkBox(self.integer, e = 1 , v = 0)

## enables the color slider checkBox
	def sfr_enableColSlCheckBox(self,args):
		self.sfr_queryAll()
		if 'float3' in self.attType:
			cmds.checkBox(self.colorSlider, e = 1, enable = 1)
		else:
			cmds.checkBox(self.colorSlider, e = 1, enable = 0)
...
# this part defines all the procedures to add attributes based on type
	...
	def sfr_addFloat3Attr(self):
		...
		if not self.useTheCol == 1:
			...
			for r in rel:
				if not cmds.objExists('%s.%s%s' % (r, self.arn, self.attrName)):
					cmds.addAttr(r, ln = '%s%s' % (self.arn, self.attrName), usedAsColor = True, at = 'float3')
					cmds.addAttr(r, ln = '%s%s%s' % (self.arn, self.attrName, self.X), at = 'float', p = '%s%s' % (self.arn, self.attrName))
					cmds.addAttr(r, ln = '%s%s%s' % (self.arn, self.attrName, self.Y), at = 'float', p = '%s%s' % (self.arn, self.attrName))
					cmds.addAttr(r, ln = '%s%s%s' % (self.arn, self.attrName, self.Z), at = 'float', p = '%s%s' % (self.arn, self.attrName))
			else:
...
# this part defines the UI of the script
	def assignAttrUI(self):
		...
		cmds.optionMenuGrp(self.menuGrp, l = 'type', cc = partial(self.sfr_enableColSlCheckBox))
		cmds.menuItem('float')
		...
		cmds.checkBox(self.colorSlider, l = 'color slider', v = 0, onc = partial(self.sfr_turnOffRandInt), enable = 0)
